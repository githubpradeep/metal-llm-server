...
| `TURBOQUANT_PREFILL_Q4` | **Opt-in only** (`=1`): Q4-flash whole prompt then spill. Default **off** — past-hot uses native TQ causal attention. |
...
**Speed model:** While `attn_kv_seq ≤ TURBOQUANT_HOT_WINDOW`, decode stays on the Q4 flash stack. Past-hot **prefill** batch-rotates and packs K/V, then uses **`turboquant_attn_v3_causal`** to share packed K/V across four query rows. Q/O rotations use the parallel f32 TQ rotation kernel: this preserves decode precision without the quality loss from f16 rotation or the latency of repeating rotation inside every causal TG. Spill once at the hot boundary. `TURBOQUANT_PREFILL_Q4=1` is an explicit escape hatch only.

**`HOT_WINDOW=0` (pure TQ):** Parallel prefill + fused decode stay enabled. K/V are dual-written as V3 (compressed store) and a model-frame Q4 attention mirror; both prefill and decode attend with Q4 flash. The mirror is sized to `LLAMA_CTX_SIZE` (~2.4 GB @ 200k on E2B) — prefer `HOT_WINDOW=2048` unless you need pure TQ.

**`--serve`:** Per-slot hot rings + residual + spill flag live in `KvCachePool`. Multi-slot TQ runs serially per slot. Past-hot prefill uses native causal TQ attention on each slot’s cold cache.
...
| **P3g** | Shipped | `turboquant_attn_v3_causal`: multi-query TQ flash with f32 in-kernel Q/O rotation |
| **P3h** | Shipped | Native TQ prefill: batched K/V Haar + packing, four-query causal attention |
