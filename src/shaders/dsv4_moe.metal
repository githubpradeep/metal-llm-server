...
// Truncated grid — full 256 entries loaded from host buffer when needed.
// For the kernel we embed the full table (same ggml constants as CPU path).
    ...
    uint gid [[thread_position_in_grid]])
{
    ...
    float sp;
    if (x > 20.0f) sp = x;
    else if (x < -20.0f) sp = exp(x);
    else sp = log(1.0f + exp(x));
    probs[i] = sqrt(sp);
}
