...
/// Prefill MLP mul_mm with f16 RHS activations (cast before each matmul).
/// Default on — set PREFILL_MLP_F16=0 to force f32 src1.
pub fn prefill_mlp_f16_enabled() -> bool {
    !matches!(
        std::env::var("PREFILL_MLP_F16").as_deref(),
        Ok("0") | Ok("false") | Ok("FALSE")
    )
}
...
pub struct MetalContext {
    ...
    mul_mm_q6k_f16_pipeline: OnceLock<ComputePipelineState>,
    /// Lazy: Q4_K mul_mm with f16 RHS + f16 dst.
    ...
    pub rmsnorm_acc_batch_pipeline: ComputePipelineState,
    ...
    pub cast_f32_to_f16_pipeline: ComputePipelineState,
    ...
    pub gelu_mul_stacked_batch_f16_pipeline: ComputePipelineState,
    pub gelu_mul_stacked_batch_f16_from_f32_pipeline: ComputePipelineState,
    ...
    /// TurboQuant: Y = X @ M batched `head_dim × head_dim` rotation.
    pub turboquant_rotate_pipeline: ComputePipelineState,
    /// TurboQuant V3 (2/3-bit): quantize rotated vectors to norm + packed indices.
...
impl MetalContext {
    pub fn new() -> Self {
        ...
        let turboquant_path =
            Path::new(env!("CARGO_MANIFEST_DIR")).join("src/shaders/turboquant.metal");
        let mut shader_src =
        ...
        shader_src.push('\n');
        shader_src.push_str(
            &std::fs::read_to_string(&turboquant_path).expect("Failed to read turboquant.metal"),
        );
        ...
        let rmsnorm_acc_batch_pipeline = get_fn("rmsnorm_acc_batch");
        ...
        let cast_f32_to_f16_pipeline = get_fn("cast_f32_to_f16");
        ...
        let gelu_mul_stacked_batch_f16_pipeline = get_fn("gelu_mul_stacked_batch_f16");
        let gelu_mul_stacked_batch_f16_from_f32_pipeline =
            get_fn("gelu_mul_stacked_batch_f16_from_f32");
        ...
        MetalContext {
            ...
            gelu_mul_stacked_batch_f16_pipeline,
            gelu_mul_stacked_batch_f16_from_f32_pipeline,
    ...
    /// Prefill mul_mm with f16 RHS (`mul_mm_q*_K_f16`). `x_buf` must hold halfs.
    pub fn encode_mul_mm_kquant_f16_at_view(
        &self,
        encoder: &metal::ComputeCommandEncoderRef,
        weight: &BufferView,
        x_f16_buf: &Buffer,
        ...
        use crate::ggml_gemv::{
            mul_mm_args_k_f16, mul_mm_dispatch, MUL_MM_SMEM, Q4_K_BLOCK_BYTES, Q6_K_BLOCK_BYTES,
        };
        let (pipeline, block_bytes) = match weight.format {
            weight_fmt::Q4_K => (self.mul_mm_q4k_f16_pipeline(), Q4_K_BLOCK_BYTES),
            weight_fmt::Q6_K => (self.mul_mm_q6k_f16_pipeline(), Q6_K_BLOCK_BYTES),
            other => panic!("encode_mul_mm_kquant_f16_at_view: not K-quant ({})", other),
        };
        let args = mul_mm_args_k_f16(m, k, seq_len, block_bytes);
        encoder.set_compute_pipeline_state(pipeline);
        encoder.set_bytes(
            0,
            std::mem::size_of::<crate::ggml_gemv::GgmlMulMmArgs>() as u64,
        ...
        encoder.set_buffer(1, Some(&weight.buffer), weight.offset);
        encoder.set_buffer(2, Some(x_f16_buf), 0);
        encoder.set_buffer(3, Some(y_buf), 0);
        encoder.set_threadgroup_memory_length(0, MUL_MM_SMEM);
        let (tg_x, tg_y, tg_z, tw, nsg) = mul_mm_dispatch(m, seq_len);
        encoder.dispatch_thread_groups(
    ...
    /// Prefill mul_mm with f16 RHS and f16 dst. `x_f16_buf` / `y_f16_buf` hold halfs.
    pub fn encode_mul_mm_kquant_f16_f16_at_view(
        &self,
        encoder: &metal::ComputeCommandEncoderRef,
        weight: &BufferView,
        x_f16_buf: &Buffer,
        ...
        let (pipeline, block_bytes) = match weight.format {
            weight_fmt::Q4_K => (self.mul_mm_q4k_f16_f16_pipeline(), Q4_K_BLOCK_BYTES),
            weight_fmt::Q6_K => (self.mul_mm_q6k_f16_f16_pipeline(), Q6_K_BLOCK_BYTES),
            other => panic!("encode_mul_mm_kquant_f16_f16_at_view: not K-quant ({})", other),
        };
        let args = mul_mm_args_k_f16(m, k, seq_len, block_bytes);
        encoder.set_compute_pipeline_state(pipeline);
        encoder.set_bytes(
        ...
        encoder.set_buffer(1, Some(&weight.buffer), weight.offset);
        encoder.set_buffer(2, Some(x_f16_buf), 0);
        encoder.set_buffer(3, Some(y_f16_buf), 0);
        encoder.set_threadgroup_memory_length(0, MUL_MM_SMEM);
        let (tg_x, tg_y, tg_z, tw, nsg) = mul_mm_dispatch(m, seq_len);
        encoder.dispatch_thread_groups(
            metal::MTLSize::new(tg_x, tg_y, tg_z),
    ...
    pub fn encode_cast_f32_to_f16(
        ...
        let tg = 256u64;
        encoder.dispatch_thread_groups(
            metal::MTLSize::new(((n as u64) + tg - 1) / tg, 1, 1),
    ...
    pub fn encode_gelu_mul_stacked_batch_f16(
        &self,
        encoder: &metal::ComputeCommandEncoderRef,
        gate_up_stacked_f16: &Buffer,
        out_f16: &Buffer,
        m: u32,
        seq_len: u32,
    ) {
        encoder.set_compute_pipeline_state(&self.gelu_mul_stacked_batch_f16_pipeline);
        encoder.set_buffer(0, Some(gate_up_stacked_f16), 0);
        encoder.set_buffer(1, Some(out_f16), 0);
        encoder.set_bytes(2, 4, &m as *const u32 as *const _);
        encoder.set_bytes(3, 4, &seq_len as *const u32 as *const _);
        let total = (seq_len * m) as u64;
        encoder.dispatch_threads(MTLSize::new(total, 1, 1), MTLSize::new(256, 1, 1));
    ...
    pub fn encode_gelu_mul_stacked_batch_f16_from_f32(
        &self,
        encoder: &metal::ComputeCommandEncoderRef,
        gate_up_stacked: &Buffer,
        out_f16: &Buffer,
        ...
        encoder.set_compute_pipeline_state(&self.gelu_mul_stacked_batch_f16_from_f32_pipeline);
        encoder.set_buffer(0, Some(gate_up_stacked), 0);
        encoder.set_buffer(1, Some(out_f16), 0);
        encoder.set_bytes(2, 4, &m as *const u32 as *const _);
        encoder.set_bytes(3, 4, &seq_len as *const u32 as *const _);
    ...
    /// Prefill: `acc[row] += RMSNorm(x[row]) * weight` for `seq_len` rows.
    pub fn encode_rmsnorm_acc_batch_view(
        &self,
        encoder: &metal::ComputeCommandEncoderRef,
        acc_buf: &Buffer,
        x_buf: &Buffer,
        ...
        encoder.set_compute_pipeline_state(&self.rmsnorm_acc_batch_pipeline);
    ...
    /// TurboQuant rotation: `out = in @ rot_matrix`, one row per (head, token).
    /// `in`/`out` are `[rows, head_dim]` row-major f32 and must not alias.
    /// `rot_matrix` is `[head_dim, head_dim]` row-major (Rᵀ to rotate, R to invert).
    pub fn encode_turboquant_rotate(
        &self,
        encoder: &metal::ComputeCommandEncoderRef,
        in_buf: &Buffer,
        out_buf: &Buffer,
        ...
        encoder.set_compute_pipeline_state(&self.turboquant_rotate_pipeline);
        encoder.set_buffer(0, Some(in_buf), 0);
        encoder.set_buffer(1, Some(out_buf), 0);
        encoder.set_buffer(2, Some(rot_matrix), 0);
        encoder.set_bytes(3, 4, &head_dim as *const u32 as *const _);
    ...
    /// TurboQuant V3 (2/3-bit): quantize already-rotated vectors into the cache
    /// row layout (fp16 norm + bit-packed Lloyd–Max indices). One thread per row.
    pub fn encode_turboquant_quant_v3(
        &self,
        encoder: &metal::ComputeCommandEncoderRef,
        in_buf: &Buffer,
        cache: &Buffer,
        ...
        encoder.set_compute_pipeline_state(&self.turboquant_quant_v3_pipeline);
        encoder.set_buffer(0, Some(in_buf), 0);
        encoder.set_buffer(1, Some(cache), 0);
        ...
        let tg = rows.min(64).max(1);
        encoder.dispatch_thread_groups(
            MTLSize::new(((rows + tg - 1) / tg) as u64, 1, 1),
            MTLSize::new(tg as u64, 1, 1),
        );
    }

    /// TurboQuant V3 (2/3-bit): fused rotate + quantize + residual-window store.
    /// Decode: `seq_stride=1`, `token_index=0`. Prefill batch: `seq_stride=seq_len`,
    /// loop `token_index` over the chunk (layout `(rows, seq_stride, head_dim)`).
    pub fn encode_turboquant_rotate_quant_v3(
        &self,
        encoder: &metal::ComputeCommandEncoderRef,
        in_normed: &Buffer,
        cache: &Buffer,
        ...
        self.encode_turboquant_rotate_quant_v3_strided(
            encoder,
            in_normed,
            cache,
            centroids,
    ...
    pub fn encode_turboquant_rotate_quant_v3_strided(
        &self,
        encoder: &metal::ComputeCommandEncoderRef,
        in_normed: &Buffer,
        cache: &Buffer,
        ...
        encoder.set_compute_pipeline_state(&self.turboquant_rotate_quant_v3_pipeline);
        encoder.set_buffer(0, Some(in_normed), 0);
        encoder.set_buffer(1, Some(cache), 0);
        encoder.set_buffer(2, Some(centroids), 0);
        encoder.set_bytes(3, 4, &head_dim as *const u32 as *const _);
        encoder.set_bytes(4, 4, &capacity as *const u32 as *const _);
        ...
        encoder.set_buffer(9, Some(window), 0);
        encoder.set_bytes(10, 4, &rw as *const u32 as *const _);
        encoder.set_buffer(11, Some(fwd), 0);
        encoder.set_bytes(12, 4, &seq_stride as *const u32 as *const _);
        encoder.set_bytes(13, 4, &token_index as *const u32 as *const _);
        let tg = head_dim.min(256).max(1);
        encoder.dispatch_thread_groups(
            MTLSize::new(rows as u64, 1, 1),
            MTLSize::new(tg as u64, 1, 1),
    ...
    pub fn encode_turboquant_spill_q4_to_v3(
        ...
        if n_tokens == 0 || rows == 0 {
            return;
        }
        encoder.set_compute_pipeline_state(&self.turboquant_spill_q4_to_v3_pipeline);
        encoder.set_buffer(0, Some(q4_cache), 0);
        encoder.set_buffer(1, Some(tq_cache), 0);
        encoder.set_buffer(2, Some(centroids), 0);
        encoder.set_buffer(3, Some(fwd), 0);
        encoder.set_buffer(4, Some(window), 0);
        encoder.set_bytes(5, 4, &head_dim as *const u32 as *const _);
        encoder.set_bytes(6, 4, &hot_cap as *const u32 as *const _);
        encoder.set_bytes(7, 4, &tq_cap as *const u32 as *const _);
        encoder.set_bytes(8, 4, &n_tokens as *const u32 as *const _);
        encoder.set_bytes(9, 4, &bits as *const u32 as *const _);
        encoder.set_bytes(10, 4, &q4_row_bytes as *const u32 as *const _);
        encoder.set_bytes(11, 4, &tq_row_bytes as *const u32 as *const _);
        encoder.set_bytes(12, 4, &rows as *const u32 as *const _);
        encoder.set_bytes(13, 4, &rw as *const u32 as *const _);
        let tg = head_dim.min(256).max(1);
        let n_tg = (rows as u64) * (n_tokens as u64);
        encoder.dispatch_thread_groups(
            MTLSize::new(n_tg, 1, 1),
            MTLSize::new(tg as u64, 1, 1),
        );
    ...
    /// TurboQuant V3 (2/3-bit): fused single-token attention.
    pub fn encode_turboquant_attn_v3(
        &self,
        encoder: &metal::ComputeCommandEncoderRef,
        q_buf: &Buffer,
        k_cache: &Buffer,
    ...
    pub fn encode_prefill_attention_causal_q4_0(
        ...
        {
            if let (Some(scratch), Some(layout), Some(q_f16)) =
                (fa_ext_scratch, fa_ext_layout, q_f16_scratch)
            {
